...
//Added 10 to all parseInt functions to elimnate need to search for integer base.
...
var resizePizzas = function(size) {
  ...
  //Minimized DOM touches inside for loop by creating variables and moving
  //them outside the loop.
  function changePizzaSizes(size) {
    var pizzaContainer = document.getElementsByClassName("randomPizzaContainer");
    var numPizzas = pizzaContainer.length;
...
//Create pizzas in memory using Document Fragment so the docfrag pizzas can be
//appended to the DOM once outside the for loops to improve performance
var docfragPizzas = document.createDocumentFragment();
var pizzasDiv = document.getElementById("randomPizzas");
// This for-loop actually creates and appends all of the pizzas when the page loads

for (var i = 0; i < 100; i++) {
  docfragPizzas.appendChild(pizzaElementGenerator(i));
}

pizzasDiv.appendChild(docfragPizzas);
...
//Moved items related variables outside for loop to improve performance
//Moved phase calculations outside loop to reduct DOM touches and number
//of calculations.
  var items = document.getElementsByClassName('mover');
  var itemsLength = items.length;
  var scrollValue = document.body.scrollTop / 1250;
  var phaseVal = [
    Math.sin(scrollValue),
    Math.sin(scrollValue + 1),
    Math.sin(scrollValue + 2),
    Math.sin(scrollValue +3),
    Math.sin(scrollValue + 4)
    ];
//Changed to translate3d transformation to improve performance
  for (var i = 0; i < itemsLength; i++) {
    phase = phaseVal[i%5];
 //   items[i].style.transform = "translateX(" + 100 * phase + "px)";
      items[i].style.transform = "translate3d(" + 100 * phase + "px, " + 100 * phase + "px, 0)";
  }
...
//Again, using DocFrag to create the pizzas in memory first and
//append to the DOM tree just once
var docfragPizzaTree = document.createDocumentFragment();

function generatePizzas (i, cols, s, h) {
    ...
    docfragPizzaTree.appendChild(elem);
}
document.addEventListener('DOMContentLoaded', function() {
  ...
  var numberPizzas = cols * 4
//  if (screen.width < 400) {
//    h = h * 4;
//    numberPizzas = numberPizzas * 2;
//  }

  for (var i = 0; i < numberPizzas; i++) {
    generatePizzas(i, cols, s, h);
  }

  movingPizzas.appendChild(docfragPizzaTree);
});