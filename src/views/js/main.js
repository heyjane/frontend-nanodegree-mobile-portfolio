...
//Added radix value of 10 to specify decimal system numeration for all parseInt()functions
...
var resizePizzas = function(size) {
  ...
  //Per jsperf.com tests, changed querySelectorAll to getElementById to improve performance
  //Created a variable to hold the random pizzas array outside the for loop
  var randomPizzaArray = document.getElementsByClassName("randomPizzaContainer");
  //Moved variables outside the for loop since you only need to look at the first pizza
  //in the array to determine the size difference (dx) and then calculate the
  //new width (newwidth) for all the pizzas.
  var dx = determineDx(randomPizzaArray[0], size);
  var newwidth = (randomPizzaArray[0].offsetWidth + dx) + 'px';
  var numRandomPizzas = randomPizzaArray.length
...
//Replaced original for-loop that appended elements for each iteration with
//a new loop that first creates all the pizzas in memory with createDocumentFragment
//so we can append only once to improve performance.
var newPizzas = document.createDocumentFragment();
var pizzasDiv = document.getElementById("randomPizzas");

for (var i = 2; i < 100; i++) {
  newPizzas.appendChild(pizzaElementGenerator(i));
}
pizzasDiv.appendChild(newPizzas);
